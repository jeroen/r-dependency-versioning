...
\newcommand{\JavaScript}{\texttt{Java}\xspace}
...
\title{Possible Directions for Improving Dependency Versioning in R}
...
times per year ships with the \dfn{base} and \dfn{recommended} packages and
...
R Archive Network'' (\CRANpkg{CRAN}), featuring over 4300 contributed packages as
...
The \emph{R Core team} has done a tremendous job in coordinating the
...
network of recursive dependencies. Each package includes a \file{DESCRIPTION}
file which allows for declaration of several types of dependencies, including
\code{Depends}, \code{Imports}, \code{Suggests} and \code{Enhances}. Based on
...
management is also related to the issue of \dfn{namespacing}, because different
packages can use identical names for objects. The \file{NAMESPACE} file allows
...
or loading a package. The \code{install.packages} function downloads and
...
without warning. Similarly, the \code{library} function will load the earliest
found package with a matching name.

The \file{DESCRIPTION} file does allow the package author to specify a certain
...
The term \dfn{dependency} in this context does not exclusively refer to formally
...
\subsection{Case 1: Archive / repository maintenance}
...
recent example was the forced roll-back of the \pkg{ggplot2} \citep{ggplot2}
update to version 0.9.0, because the introduced changes caused several other
packages to break. The author of the \pkg{ggplot2} package has since been
required to announce upcoming updates to authors of packages that depend on
\pkg{ggplot2}, and provide a release candidate to test compatibility. The
...
created with R 2.13 and loads the \pkg{caret} package \citep{caret}. If no further
instructions are provided, this means that any of the approximately 25 releases
of \pkg{caret} in the life cycle of R 2.13 (April 2011 to February 2012) could
have been used, making reproducibility unlikely. Sometimes authors add comments
in the code where the package is loaded, stating that e.g. \pkg{caret 4.78} was
used. However, this information might also turn out to be insufficient:
\pkg{caret} depends on 4 packages, and suggests another 59 packages, almost all of which
have had numerous releases in R 2.13 time frame.
Consequently, \pkg{caret 4.78} might not work anymore because of changes in
...
\pkg{caret 4.78} release. Instead, lets assume that the prescient researcher
anticipated all of this, and saved the full output of \code{sessionInfo()}
...
available for R 2.13 are respectively \pkg{caret 5.13} on Windows, and
\pkg{caret 5.14} on OSX. Most likely, they will face the task of rebuilding
...
\subsection{Case 3: Production applications}
...
A \dfn{software distribution} (also referred to as a \dfn{distribution} or a \dfn{distro})
...
\dfn{beta} releases of the distribution. However, software versions in the testing branch will
...
as a distribution of the 29 \code{base} and \code{recommended} packages.
...
consider a \dfn{development} or \dfn{unstable} branch. It consists of the pool
of \dfn{bleeding-edge} versions, straight from package authors. Consequently it
is wise to assume that software in this branch might break on a regular
basis. Usually, the main purpose of an
\dfn{unstable} branch is for developers to exchange new versions and test
compatibility of software. Regular users obtain software releases from
\dfn{stable} branches instead. This does not sound unfamiliar: the r-base
software also distinguishes between stable versions \dfn{r-release} and
\dfn{r-release-old}, and an unstable development version, \dfn{r-devel}.
...
with every version of \dfn{r-release}, and new package updates would only be
published to the \dfn{r-devel} branch. In practice, this could perhaps quite
...
\dfn{stable} for this release. In the case of binary packages for OSX and
...
\dfn{devel} and \dfn{testing} branches; they should not affect \dfn{stable}
...
to as \dfn{backporting}, but this should only happen incidentally.
...
example by adding an additional parameter to \code{install.packages} named
\code{devel=TRUE}. However when installing an unstable package, it must be
...
the node.js software (for short: \dfn{node}), a Javascript server system based
on the open source engine \dfn{V8} from Google. One of the reasons that the
community has been able to grow rapidly is because of the excellent package
manager and identically named repository, \dfn{NPM}. Even though this package
...
Below a snippet taken from the \file{package.json} file in the NPM manual:
...
branch of R. This way, the \dfn{stable} versions of R are tied to a fixed

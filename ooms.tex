...
R Archive Network'' (\dfn{CRAN}), featuring over 4300 contributed packages as
...
of recursive dependencies. Each package includes a \file{DESCRIPTION} file
...
warning. Similarly, the \code{library} function will load earliest found
...
clear solution.
One recent example was the forced roll-back of the \pkg{ggplot2}
\citep{ggplot2} update to version 0.9.0, because the introduced changes caused several other packages to break. The maintainer of the \pkg{ggplot2} package has since been
...
created with R 2.13 and loads the \pkg{caret} package. If no further
...
consider a \dfn{development} or \dfn{unstable} branch. It is the pool of
\dfn{bleeding edge} packages, straight from the developers.
%
% As a result, it is wise to assume that software in this branch might break on a
% regular basis, as a result of dependency changes. Regular users are not supposed
% to use software from unstable branches; it is used only by developers to share
% latest versions of their software and test compatibility and dependencies.
% Regular users should obtain software releases from \dfn{stable} branches. This
% already sounds familiar: the r-base software also distinguished between stable
% versions \dfn{r-release} and \dfn{r-release-old}, and an unstable development
% version, \dfn{r-devel}.
%
...
\dfn{devel} and \dfn{testing} branches; they should not affect previous
...
%The core challenges to be addressed are
%support for loading of multiple versions of a package, and introducing a
%notation for declaring and importing of explicitly versioned dependencies.
